...
const LoginIndustry = () => {
    ...
    const handleLogin = async (e) => {
      ...
      setError('');
      try {
        const { data } = await axios.post(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}api/auth/login-industry`, { phone_no, password });
        localStorage.setItem('token', data.token);
        navigate('/industry-dashboard'); // Redirect to dashboard or home page
      } catch (err) {
    ...
    return (
      <div className="mx-auto max-w-screen-xl h-full px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-lg">
          <h1 className="text-center text-2xl font-bold text-indigo-600 sm:text-3xl">Login to your industry account</h1>
